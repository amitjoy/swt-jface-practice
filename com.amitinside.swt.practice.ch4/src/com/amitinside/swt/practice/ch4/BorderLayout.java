 ...
 * positioning of controls. To position controls, call
 * <code>control.setLayoutData()</code>, passing the <code>BorderData</code> of
 * your choice.
 * 
 * For example:
 * 
...
public class BorderLayout extends Layout {
	 ...
	 * 
	 * @param composite
	 *            the composite that contains the controls
	 * @param wHint
	 *            width hint in pixels for the minimum width
	 * @param hHint
	 *            height hint in pixels for the minimum height
	 * @param flushCache
	 *            if true, flushes any cached values
	 * @return Point
	...
	protected Point computeSize(Composite composite, int wHint, int hHint,
		...
		if (north != null) {
			final Point pt = getSize(north, flushCache);
			width = Math.max(width, pt.x);
		}
		if (south != null) {
			final Point pt = getSize(south, flushCache);
			width = Math.max(width, pt.x);
		}
		...
		if (west != null) {
			final Point pt = getSize(west, flushCache);
			heightOther = Math.max(heightOther, pt.y);
		}
		if (east != null) {
			final Point pt = getSize(east, flushCache);
			heightOther = Math.max(heightOther, pt.y);
		}
		height += heightOther;
	...
	protected void getControls(Composite composite) {
		...
		for (int i = 0, n = children.length; i < n; i++) {
			...
			final BorderData borderData = (BorderData) child.getLayoutData();
